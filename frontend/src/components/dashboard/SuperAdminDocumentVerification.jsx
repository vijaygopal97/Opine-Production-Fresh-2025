...
const SuperAdminDocumentVerification = () => {
  ...
  const fetchPendingProfiles = async () => {
    try {
      setLoading(true);
      const response = await authAPI.getIndependentInterviewerProfiles();
      setPendingProfiles(response.data || []);
    } catch (error) {
      console.error('Error fetching pending profiles:', error);
      showError('Failed to fetch pending profiles');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReview = async (status) => {
    ...
    try {
      setReviewing(true);
      await authAPI.reviewIndependentInterviewerProfile(selectedProfile._id, {
        status,
  ...
  return (
    <div className="w-full">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">Review and approve independent interviewer profile submissions</p>
      </div>
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200 flex-shrink-0">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  {previewType === 'Bank Document' && previewBankDetails && (
                    <div className="text-sm text-gray-600 mt-1 space-y-1">
                      <p><span className="font-medium">Account Number:</span> {previewBankDetails.accountNumber || 'Not provided'}</p>
                      <p><span className="font-medium">Account Holder:</span> {previewBankDetails.accountHolderName || 'Not provided'}</p>
                      <p><span className="font-medium">Bank Name:</span> {previewBankDetails.bankName || 'Not provided'}</p>
                      <p><span className="font-medium">IFSC Code:</span> {previewBankDetails.ifscCode || 'Not provided'}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="flex-1 overflow-hidden p-6">
              <div className="h-full flex items-center justify-center bg-gray-50 rounded-lg">
                ...
                <div className="hidden flex-col items-center justify-center text-center p-8">
                  ...
                  <p className="text-gray-600">The document could not be loaded for preview.</p>
                </div>
              </div>
            </div>
          </div>
...
export default SuperAdminDocumentVerification;